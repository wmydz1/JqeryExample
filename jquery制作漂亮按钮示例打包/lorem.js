...
function getLorem(type, length, tag, selectID){
    ...
    // Start at random dot
    startPos = randomSentancePosition(lorem);
    
    // Search for words
    lastIndexOf = startPos;
    for(var i = 0; i < length; i++){
        endPos = lorem.indexOf(" ", lastIndexOf+1);
        lastIndexOf = endPos;
    }
    output = lorem.slice(startPos, endPos);
    ...
    if(selectID == "list"){
        ...
        var listitem = "";
        for(var i = 0; i < length; i++){
            listitem = getWords(lorem, 4+Math.floor(Math.random()*8));
            lastchar = listitem[listitem.length-1];
            if(lastchar == "." || lastchar == ","){
                listitem = listitem.slice(0, (listitem.length-1));
            }
            if(tag != false) { listitem = wrapInTag(listitem, "li"); } else { listitem += "\n"; }
            listoutput += listitem;
        }
        output = listoutput;
    }
